...
extract_annotations <- function(file_path, wide_format = FALSE) {
  ...
  while (iterations < max_iterations && 
    ...
    annotations_with_adjusted_time_slots <- annotations_with_adjusted_time_slots %>%
      rowwise() %>%
      mutate(parent_time_slots = list(find_parent_time_slots(ANNOTATION_REF, annotations_with_adjusted_time_slots)),
...
# example for testing
annotations_df <- extract_annotations("internal/ELAN_test_files/280117_10_Hamid_Clandividing.eaf", wide_format = FALSE)
